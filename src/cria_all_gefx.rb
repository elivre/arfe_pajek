...
def cria_gefx(dir_gefx, arq_net_grafo)
Encoding.default_internal = Encoding::ISO_8859_1
Encoding.default_external = Encoding::UTF_8

 #-------------------------------------------------
 #  determina nome de municipio, cargo e ano em arq_net_grafo
 #------------------------------------------------ 
  nomes=File.basename(arq_net_grafo,'.net').split(/-/)
 ...
 #------------------------------------------------
  ...
  arq_gefx =  "#{dir_gefx}/#{File.basename(arq_net_grafo).gsub('.net','.gefx')}"
  
  fr  = File.open(arq_net_grafo,'r')
...
def gera_all_gefx(dir_draw, dir_gefx)
  Dir.chdir(dir_draw)
  Dir.glob("*.net") do |arq_net_grafo|
    cria_gefx(dir_gefx, "#{dir_draw}/#{arq_net_grafo}")
  end
end
...
dir_gefx = "/home/neilorca/TSE/arfe2012/RedesCargoUE_2012/draw-gefx"
dir_draw = "/home/neilorca/TSE/arfe2012/RedesCargoUE_2012/draw"

Dir.mkdir(dir_gefx)
gera_all_gefx(dir_draw, dir_gefx)
